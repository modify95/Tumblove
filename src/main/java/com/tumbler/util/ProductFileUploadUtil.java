...
@Log
public class ProductFileUploadUtil {   
   ...
   public static String fileUpload(MultipartFile file, HttpServletRequest request, String fileName) throws IOException {
      ...
      /*if (newName.length()==0) throw new IllegalStateException("File Name " + org_name + " results in a empty fileName!");
      return newName;*/
      
      // 파일명 변경(중복되지 않게) 
      if(org_name != null && (!org_name.equals(""))){
         real_name = (fileName + "_" + org_name).replaceAll("[^a-zA-Z0-9_]*", ""); // 저장할 파일 이름
   ...
   public static void fileDelete(String fileName, HttpServletRequest request) throws IOException{
      ...
      String docRoot = request.getSession().getServletContext().getRealPath("/upload/"+dirName);
   ...
   public static String makeThumbnail(String fileName, HttpServletRequest request) throws Exception{
      String dirName = fileName.substring(0, fileName.indexOf("_")); 
      // 이미지가 존재하는 폴더 추출
      String imgPath = request.getSession().getServletContext().getRealPath("/upload/"+dirName);
