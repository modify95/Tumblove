$(function(){
	...
	$('#m_id, #m_password').bind("keyup",function(){
	...
	$("#loginBtn").click(function(){
		...
		else if (!formCheck($('#m_password'),$('.error:eq(1)'), "비밀번호를"))	return;
		else if (!inputVerify(1,'#m_password', '.error:eq(1)'))	return;
		else {
			$("#loginForm").attr({
				"method":"POST",
				"action":"/member/login/login"
			});
			$("#loginForm").submit();
		}
	});
